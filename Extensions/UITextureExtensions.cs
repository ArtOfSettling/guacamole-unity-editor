...
namespace WellFired.Guacamole.Unity.Editor
{
	public static class Texture2DExtensions
	{
		...
		public static Texture2D CreateRoundedTexture(int width, int height, UIColor backgroundColor, UIColor outlineColor, double radius, CornerMask cornerMask)
		{
			var result = new Texture2D(width, height)
			{
			...
			if (topRight)
			{
				...
				path.AddLine(new Vector(width, radius), bottomRight ? new Vector(width, height - radius) : new Vector(width, height));
			}
			else
			{
				path.AddLine(new Vector(width, 0), bottomRight ? new Vector(width, height - radius) : new Vector(width, height));
			}
			...
			if (bottomRight)
			{
				...
				path.AddLine(new Vector(width - radius, height), bottomLeft ? new Vector(radius, height) : new Vector(0, height));
			}
			else
			{
